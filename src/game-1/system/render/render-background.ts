...
function renderOutsideCoordinates(
    { rows, columns, cellSizePx, gridOffsetPx }: Dimensions,
    ...
    for (let row = 0; row < rows; row++) {
        ...
        ctx.fillText(
            row.toString(10),
            columns * cellSizePx + gridOffsetPx + cellSizePx / 2,
            gridOffsetPx + cellSizePx * row + cellSizePx / 2,
        )
    }
    ...
    for (let column = 0; column < columns; column++) {
        ...
        ctx.fillText(
            column.toString(10),
            gridOffsetPx + cellSizePx * column + cellSizePx / 2,
            rows * cellSizePx + gridOffsetPx + cellSizePx / 2,
        )
    }
...
function renderTerrain(
    world: IWorld,
    { gridOffsetPx, cellSizePx }: Dimensions,
    ctx: CanvasRenderingContext2D,
): void {
...
function renderOutsideBox(
    world: IWorld,
    { columns, rows, gridOffsetPx, cellSizePx }: Dimensions,
    ctx: CanvasRenderingContext2D,
): void {
    ctx.save()

    ctx.strokeStyle = color.outsideBox
    ctx.strokeRect(gridOffsetPx, gridOffsetPx, columns * cellSizePx, rows * cellSizePx)

    ctx.restore()
}

function renderGrid(
    { rows, columns, gridOffsetPx, cellSizePx }: Dimensions,
    ctx: CanvasRenderingContext2D,
): void {
    ctx.save()
    ...
    for (let row = 1; row < rows; row++) {
        ...
        ctx.lineTo(gridOffsetPx + columns * cellSizePx, gridOffsetPx + row * cellSizePx)
        ctx.stroke()
    }
    ...
    for (let column = 1; column < columns; column++) {
        ...
        ctx.lineTo(gridOffsetPx + column * cellSizePx, gridOffsetPx + rows * cellSizePx)
        ctx.stroke()
    }
...
export function renderBackground(
    ...
    renderOutsideBox(world, dimensions, ctx)
    renderCoordinates(dimensions, ctx)
    renderOutsideCoordinates(dimensions, ctx)
    renderTerrain(world, dimensions, ctx)
    renderObstructions(world, dimensions, ctx)
}
